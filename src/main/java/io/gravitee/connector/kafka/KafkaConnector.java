...
public class KafkaConnector
    ...
    static final String KAFKA_CONTEXT_ATTRIBUTE =
            ExecutionContext.ATTR_PREFIX + "kafka.";

    static final String CONTEXT_ATTRIBUTE_KAFKA_OFFSET =
            KAFKA_CONTEXT_ATTRIBUTE + "offset";
    static final String CONTEXT_ATTRIBUTE_KAFKA_PARTITION =
            KAFKA_CONTEXT_ATTRIBUTE + "partition";
    static final String CONTEXT_ATTRIBUTE_KAFKA_TOPIC =
            KAFKA_CONTEXT_ATTRIBUTE + "topic";
    static final String CONTEXT_ATTRIBUTE_KAFKA_CLIENT_ID =
            KAFKA_CONTEXT_ATTRIBUTE + CommonClientConfigs.CLIENT_ID_CONFIG;
    ...
    private void handleWebsocketRequest(ExecutionContext context,
                                        ProxyRequest request,
                                        String topic,
                                        Handler<Connection> connectionHandler) {
                                    ...
                                    ((WebSocketProxyRequest) request).upgrade()
                                            .thenAccept(
                                                    new Consumer<WebSocketProxyRequest>() {
                                                        @Override
                                                        public void accept(
                                                                WebSocketProxyRequest webSocketProxyRequest
                                                        ) {
                                                            WebsocketConnection connection;

                                                            if (partition == -1) {
                                                                connection =
                                                                        new TopicBasedWebsocketConnection(
                                                                                consumer,
    ...
    private void handleRequest(ExecutionContext context,
                               ProxyRequest request,
                               String topic,
                               Handler<Connection> connectionHandler) {
    ...
    private KafkaProducer<String, String> createProducer(ExecutionContext context) {
        ...
        return create((Function<Map<String, String>, KafkaProducer>) config1 -> KafkaProducer.create(Vertx.currentContext().owner(), config1), config);
    }
    ...
    private KafkaConsumer<String, String> createConsumer(
        ...
        return create((Function<Map<String, String>, KafkaConsumer>) config1 -> KafkaConsumer.create(Vertx.currentContext().owner(), config1), config);
    }
    ...
    private boolean isWebSocket(ProxyRequest request) {
        String connectionHeader = request
                .headers()
                .getFirst(HttpHeaders.CONNECTION);
    ...
    private String getClientId(ExecutionContext context) {
        String clientId = (String) context.getAttribute(
                CONTEXT_ATTRIBUTE_KAFKA_CLIENT_ID
        );
        if (clientId == null) {
    ...
    private String getGroupId(ExecutionContext context) {
        String groupId = (String) context.getAttribute(
                CONTEXT_ATTRIBUTE_KAFKA_GROUP_ID
        );
        ...
        if (groupId == null || groupId.isEmpty()) {
            ...
            if (groupId == null || groupId.isEmpty()) {
                groupId =
                        context.request().parameters().getFirst(KAFKA_GROUP_QUERY_PARAMETER);
            }
     ...
     * @return
     */
    private String extractPartition(
            ExecutionContext context,
            ProxyRequest request
    ) {
        String partition = (String) context.getAttribute(
                CONTEXT_ATTRIBUTE_KAFKA_PARTITION
        );
        if (partition == null || partition.isEmpty()) {
            partition = request.headers().getFirst(KAFKA_PARTITION_HEADER);
            if (partition == null || partition.isEmpty()) {
                partition =
                        request.parameters().getFirst(KAFKA_PARTITION_QUERY_PARAMETER);
            }
        }
    ...
    private String extractOffset(ExecutionContext context, ProxyRequest request) {
        String offset = (String) context.getAttribute(
                CONTEXT_ATTRIBUTE_KAFKA_OFFSET
        );
